...
- **Deactivate access** retains history and revokes sessions. HR termination also blocks access. Owners must replace legacy PINs with a password on first use after deployment. Authenticator codes and recovery codes are no longer required or accepted as sign-in credentials. Existing owner passwords continue working. Sessions expire after 12 hours; the last owner cannot be deactivated. Workers retain PIN sign-in. Password changes require the current password/PIN and revoke existing sessions.
