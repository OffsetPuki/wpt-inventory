...
export function useMarketingPlace<T extends string>(key:string,fallback:T){
 ...
 const change=(next:T)=>{if(key==='tab'){const [path,search]=location.hash.split('?'),params=new URLSearchParams(search);params.set(key,next);history.pushState(history.state,'',path+'?'+params.toString());}setValue(next);};
