...
export default function SearchConnections({connection:c,bing,previousBing}:{connection:Connections;bing:BingReport;previousBing:BingReport}) {
  ...
  return <section aria-label="Search connections and visibility" className="border rounded-xl p-4 space-y-5 min-w-0">
    ...
    {(googleErrors.length>0||bing.errors.length>0)&&<div role="status" className="rounded-lg border p-3 text-sm"><p className="font-medium">Some reports could not refresh. Previously saved data remains visible.</p><ul className="list-disc pl-5">{googleErrors.map(e=><li key={'g'+e.kind}>Google {e.kind}: {e.error}</li>)}{bing.errors.map(e=><li key={'b'+e.kind}>Bing {e.kind}: {e.message}</li>)}</ul></div>}
