  ...
  return <Modal open onClose={onClose} title={`Customer activity · ${data?.number||kind}`} maxWidth="max-w-4xl">
    <div className="space-y-6" aria-label="Document activity report">
      ...
      {!data?query.isPending&&<p>Loading activity…</p>:<>
        ...
        <section className="space-y-3"><h3 className="font-semibold">Devices & location</h3><VisitContextNote/>{data.devices.map(d=><p className="text-sm" key={`${d.deviceName||d.device}:${d.browser}`}>{d.deviceName||d.device} · {d.browser}: {d.visits} {d.visits===1?'visit':'visits'}</p>)}<p className="text-xs text-muted-foreground">Loading covers the page and its images. Action errors mean a quote response or payment checkout could not be submitted.</p></section>
        <section className="space-y-3"><h3 className="font-semibold">Visit timeline</h3><p className="text-xs text-muted-foreground">Latest 100 visits. Totals include all recorded visits.</p>{!data.visits.length&&<p className="text-sm text-muted-foreground">No recorded visits yet. Share the customer link to start collecting activity.</p>}{data.visits.map(v=><details key={v.id} className="rounded-lg border border-border p-3"><summary className="cursor-pointer text-sm">{date(v.startedAt)} · {v.deviceName||v.device} / {v.browser} · {time(v.activeMs)} active</summary><div className="mt-3 space-y-2 text-xs"><VisitContext visit={v}/><p>{kind==='quote'?`Version ${v.revision}`:v.revision===data.revision?'Current invoice content':'Earlier invoice content'} · Last activity: {date(v.lastAt)}</p><p>Read: {v.scrollPct}% · Load: {(v.loadMs/1000).toFixed(1)}s · Image / action errors: {v.assetErrors} / {v.actionErrors}</p><p>Actions: {Object.entries(v.actions).filter(([,n])=>n).map(([key,n])=>`${names[key]} (${n})`).join(', ')||'None'}</p></div></details>)}</section>
      </>}
    </div>
  </Modal>;
}
