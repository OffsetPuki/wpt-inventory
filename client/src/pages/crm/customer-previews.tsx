...
function PreviewEditor({initial,previews,onClose}:{initial:Preview|null;previews:Preview[];onClose:()=>void}) {
  ...
  const openPreview=()=>{
    ...
    run(async()=>{try{const p=dirty?await save():preview;if(p){const url=p.url+'?owner=1&v='+p.version;if(tab)tab.location.replace(url);else window.location.assign(url);}}catch(e){tab?.close();throw e;}});
  };
