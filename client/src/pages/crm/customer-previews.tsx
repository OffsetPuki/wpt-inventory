...
import { Plus, Copy, ExternalLink, Pencil, Box, MessageSquare, RefreshCw } from 'lucide-react';
  ...
  return <Modal open onClose={()=>{if(!busy)onClose();}} title={initial?'Edit customer preview':'New customer preview'} maxWidth="max-w-3xl" preservesDraft={!dirty&&!file&&!label.trim()}>
    <div className="space-y-6">
      ...
      {preview&&<>
        ...
        <section className="space-y-3 border-t border-border pt-5" aria-label="Customer feedback"><h3 className="font-semibold">Customer feedback</h3>{feedback.length?feedback.map(f=><article key={f.id} className="rounded-lg border border-border p-4"><p className="text-xs text-muted-foreground">{f.optionLabel} · {new Date(f.createdAt).toLocaleString()}</p><p className="mt-2 whitespace-pre-wrap break-words text-sm">{f.message}</p><p className="mt-3 text-xs text-muted-foreground">{f.emailStatus==='sent'?'Email notification sent':f.emailStatus==='queued'?'Email notification queued':f.emailStatus==='attention'?'Email notification delayed. Check System health if it does not arrive.':'Received before email alerts were enabled.'}</p></article>):<p className="text-sm text-muted-foreground">No feedback yet. New comments appear here while the suite is open.</p>}</section>
      </>}
...
export default function CustomerPreviewsPage(){
  ...
  return <div className="mx-auto max-w-6xl p-4 sm:p-6"><Header title="Customer previews" description="Review the design with your customer before creating a quote."><button className={secondaryBtn} disabled={query.isFetching} onClick={()=>query.refetch()}><RefreshCw size={16}/>Refresh</button><button className={primaryBtn} onClick={()=>setEditing(null)}><Plus size={18}/>New preview</button></Header>
    {notifications.data&&<p className="mb-5 rounded-lg border border-border p-3 text-sm">{notifications.data.enabled?<>New feedback emails go to <strong>{notifications.data.recipient}</strong>.</>:<>Email alerts need setup. Feedback is saved here; check email configuration in System health.</>}</p>}
