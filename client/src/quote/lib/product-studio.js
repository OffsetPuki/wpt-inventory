...
export function createProductStudio(host){
 ...
 const floor=new THREE.Mesh(new THREE.PlaneGeometry(10000,10000),new THREE.ShadowMaterial({opacity:.22}));floor.rotation.x=-Math.PI/2;floor.position.y=-.015;floor.receiveShadow=true;scene.add(floor);const base=new THREE.Mesh(new THREE.PlaneGeometry(1,1),new THREE.MeshBasicMaterial({color:'#ded9cc',toneMapped:false}));base.rotation.x=-Math.PI/2;base.position.y=-.016;scene.add(base);let product=null,key='';
