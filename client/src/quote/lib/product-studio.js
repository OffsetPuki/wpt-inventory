...
export function createProductStudio(host){
 ...
 function setProduct(next){if(product){scene.remove(product);disposeProduct(product);}product=next;scene.add(next);const b=new THREE.Box3().setFromObject(next),size=b.getSize(new THREE.Vector3()),center=b.getCenter(new THREE.Vector3());const shape=size.toArray().map(n=>n.toFixed(2)).join(',');const radius=Math.max(1,size.length());
