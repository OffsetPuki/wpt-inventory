...
export default function Barndominium({state,onChange,customer}){
 ...
 return <div className="barndo-product"><FullscreenButton target={()=>root.current?.parentElement}/><button type="button" className="btn ghost" onClick={()=>setShowStudio(!showStudio)}>{showStudio?'Back to design & frame':'Customer preview'}</button>{showStudio&&<ProductStudio type="barndominium" state={state} customer={customer}/>}<div ref={root} hidden={showStudio}/></div>;
}
