...
export default function WebsitePreview({type,state}){
 ...
 function send(){const s=latest.current;const data=Object.fromEntries(Object.entries(s).filter(([,v])=>['string','number','boolean'].includes(typeof v)));
  if(type==='table'){const size=tableBaseFootprint(s);Object.assign(data,{type:s.tableType,top:s.includeTop==='yes'?'show':'hide',lengthFt:size.lengthFt-.25,widthIn:size.widthIn-3,heightIn:Number(s.frameHeightIn)+Number(s.topThicknessIn||2)});}
