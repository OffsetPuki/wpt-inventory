...
export default function WebsitePreview({type,state,customer}){
 ...
 return <div className="website-product-preview"><div className="product-preview-actions"><CreateCustomerPreview disabled={!ready} exportModel={exportModel} title={`${type[0].toUpperCase()+type.slice(1)} design`} customer={customer?.name||''}/></div>{!ready&&<p role="status">Loading website preview…</p>}<iframe ref={frame} title={`${type} website preview`} src={`${base}/customize/${type}?quotePreview=1&studio=1`} allow="fullscreen" referrerPolicy="strict-origin-when-cross-origin" />{error&&<p role="alert">{error}</p>}</div>;
}
