...
test('Website design imports, edits, persists and produces a quote specification',async({page})=>{
 ...
 await page.setViewportSize({width:1440,height:1000});await page.getByRole('button',{name:'Full screen',exact:true}).click();await expect(page.locator('.barndo-product')).toHaveJSProperty('clientWidth',await page.evaluate(()=>innerWidth));await expect.poll(()=>editor.locator('.bd-frame-stage').evaluate(el=>el.clientHeight)).toBeGreaterThan(500);await expect.poll(()=>editor.locator('.bd-frame-stage canvas').evaluate(el=>Math.abs(el.clientHeight-el.parentElement.clientHeight))).toBeLessThan(2);await page.getByRole('button',{name:'Exit full screen',exact:true}).click();await expect(editor.locator('.bd-frame-stage')).toHaveJSProperty('clientHeight',400);
 ...
 const share='ba'.repeat(24);app.sqlite.prepare('UPDATE quotes SET share_token=? WHERE id=?').run(share,row.id);const doc=await(await fetch(app.base+'/api/public/quote/'+share+'?preview=1')).json();expect(doc.quote.doc.project.label).toBe('Barndominium');expect(doc.quote.doc.specs.some(r=>['Frame','Secondary framing','Panels','Insulation'].includes(r.label))).toBe(false);expect(JSON.stringify(doc.quote.doc.specs)).not.toContain('product and price pending');expect(JSON.stringify(doc.quote.doc.specs)).toContain('Porch');
