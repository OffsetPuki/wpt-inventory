...
export function registerPublicPortalRoutes(app: Express): void {
  ...
  app.post("/api/public/quote/:token/accept", publicLimiter(30), (req, res) => {
    ...
    try {
      ...
      res.json({ ok: true, status: result.status, alreadyAccepted: result.alreadyAccepted });
    } catch (error) {
      console.error("[quote] acceptance rolled back", error);
      res.status(503).json({ ok: false, reason: "Please try again. Your acceptance has not been recorded." });
    }
  });
