import {specRows} from '../client/src/quote/data/configurators.js';
import {quoteBusiness,BUSINESSES} from '../shared/business.js';
import { issuedPayload, isQuoteExpired } from './quote-policy';
...
function compareOptions(a:any,b:any,lang='en'){
 if(a.type==='barndominium'&&b.type==='barndominium')return compareBarndo(a,b,lang);
 const aa=specRows(a.type,a.state),bb=specRows(b.type,b.state),labels=[...new Set([...aa,...bb].map((r:any)=>r.label))];
 return labels.map(label=>{const av=aa.find((r:any)=>r.label===label)?.value||'—',bv=bb.find((r:any)=>r.label===label)?.value||'—';return {label,a:av,b:bv,changed:av!==bv};});
}
...
function loadQuote(id:number){const q=db.select().from(quotes).where(eq(quotes.id,id)).get();if(!q||q.deletedAt)throw new Error('Choose two available quotes.');return q;}
function comparison(input:any){
 ...
 if(found[0].type!==found[1].type||quoteBusiness(sessions[0])!==quoteBusiness(sessions[1]))throw new Error('Choose options for the same product and business.');
 ...
 const options=found.map((quote:any,i:number)=>{
  ...
  const s=sessions[i],state=quote.type==='barndominium'?normalize(s.state):s.state;if(quote.type==='barndominium'&&validate(state).length)throw new Error('Review the building design before sending.');
  ...
  const sharedBook=JSON.parse((sqlite.prepare('SELECT price_book FROM quote_settings WHERE id=1').get() as any)?.price_book||'{}');
  const book=deepMerge(DEFAULT_PRICE_BOOK,s.priceBookSnapshot||deepMerge(sharedBook,sharedBook.businesses?.[quoteBusiness(s)]||{}));
  const lines:any=buildLineState(quote.type,state,book,s.overrides),totals:any=computeTotals(lines,{...s,minJobCharge:book.minJobCharge});
  for(const issue of quote.type==='barndominium'?scopeIssues(s,totals):[])issues.push(`Option ${i+1}: ${issue}`);
  ...
  return {id:quote.id,version:quote.version,number:quote.number,position:i+1,title,explanation,totalCents:quote.totalCents,recommended:quote.id===input.recommendedId,status:quote.status,svg:quote.type==='barndominium'?drawing(state):'',specs:doc?.specs||[],url:quote.shareToken?localizedLink(`${websiteURL()}/quote/${quote.shareToken}`,communicationContext({quoteNumber:quote.number}).lang):null};
 });
 return {found,sessions,public:{options,issues,differences:compareOptions(sessions[0],sessions[1]),deltaCents:found[1].totalCents-found[0].totalCents,customerName:a.name,email:a.email||'',lang:['en','es'].includes(a.preferredLanguage)?a.preferredLanguage:communicationContext({quoteNumber:found[0].number}).lang}};
}
 ...
 app.post('/api/quote-options/share',requireAuth,async(req,res)=>{
  try{
   ...
   const set=sqlite.transaction(()=>{
    ...
    if(!leadId){const customer=result.sessions[0].customer;leadId=db.insert(leads).values({name:customer.name,email:customer.email||null,phone:customer.phone||null,clientId:customer.clientId||null,preferredLanguage:result.public.lang,site:quoteBusiness(result.sessions[0]),source:'other',stage:'quote_sent',serviceRequested:result.found[0].type,estimatedValueCents:result.found.find((q:any)=>q.id===req.body.recommendedId)!.totalCents,notes:`Alternative quotes ${result.found.map((q:any)=>q.number).join(' / ')}`}).returning().get().id;}
    result.found.forEach((q:any,i:number)=>{const o=result.public.options[i];sqlite.prepare('INSERT INTO quote_option_members(quote_id,set_id,position,title,explanation) VALUES(?,?,?,?,?)').run(q.id,row.id,i+1,o.title,o.explanation);db.update(quotes).set({leadId,status:'sent',shareToken:q.shareToken||crypto.randomBytes(24).toString('hex'),sentAt:q.sentAt||now,payload:issuedPayload(q,now)}).where(eq(quotes.id,q.id)).run();});first=true;return row;
   })();
